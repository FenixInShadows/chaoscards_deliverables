...
ExtraCardGenConfig::ExtraCardGenConfig(const string& _name, NodeRep*& _rep)
	: name(_name), rep(_rep), encoder(nullptr), code(), min_cost(0), max_cost(10), forcing_minion(false)
{
}

ExtraCardGenConfig::ExtraCardGenConfig(const string& _name, NodeRep*& _filler_rep, int _min_cost, int _max_cost, bool _forcing_minion)
	: name(_name), rep(_filler_rep), encoder(nullptr), code(), min_cost(_min_cost), max_cost(_max_cost), forcing_minion(_forcing_minion)
{
}

ExtraCardGenConfig::ExtraCardGenConfig(const string& _name, NodeRep*& _filler_rep, CardEncoderNet* _encoder, const torch::Tensor& _code, int _min_cost, int _max_cost, bool _forcing_minion)
	: name(_name), rep(_filler_rep), encoder(_encoder), code(_code), min_cost(_min_cost), max_cost(_max_cost), forcing_minion(_forcing_minion)
{
}

ExtraCardGenConfig* MkExtraCardGenConfigWithRep(const string& name, NodeRep*& rep)
{
	return new ExtraCardGenConfig(name, rep);
}

ExtraCardGenConfig* MkExtraCardGenConfigWithCost(const std::string& name, NodeRep*& filler_rep, int cost)
{
	return new ExtraCardGenConfig(name, filler_rep, cost, cost);
}

ExtraCardGenConfig* MkExtraCardGenConfigWithLatentCode(const string& name, NodeRep*& filler_rep, CardEncoderNet* encoder, const torch::Tensor& code)
{
	return new ExtraCardGenConfig(name, filler_rep, encoder, code);
}

ExtraCardGenConfig* MkExtraCardGenConfigWithLatentCode(const string& name, NodeRep*& filler_rep, CardEncoderNet* encoder, const torch::Tensor& code, int cost, bool forcing_minion)
{
	return new ExtraCardGenConfig(name, filler_rep, encoder, code, cost, cost, forcing_minion);
}
...
{
	return torch::full({ 1 }, (long)val, torch::kLong);
}

torch::Tensor CreateVectorTensor(const vector<double>& v)
{
...
double GetSingleValFromTensor(const torch::Tensor& code)
{
	return code.template item<double>();
}

vector<double> ExtractVectorFromTensor(const torch::Tensor& t)
{
	return ExtractVectorFromTensor(t, 0, t.size(0));
}

vector<double> ExtractVectorFromTensor(const torch::Tensor& t, size_t start_index, size_t end_index_plus_one)
{
	const double* ptr = t.data_ptr<double>();
	return vector<double>(ptr + start_index, ptr + end_index_plus_one);
}

unsigned GetMaxIndex(const torch::Tensor& code)
{
	return (unsigned)(code.argmax(0).template item<long>());
}

unsigned GetBinaryIndex(const torch::Tensor& code)
{
	return (unsigned)(GetSingleValFromTensor(code) >= 0.5);
}
...
torch::Device* InitModel(torch::nn::Module& model)
{
...
torch::Device* LoadModel(torch::nn::Module& model, const char* file_path)
{
	torch::Device* device_ptr = InitModel(model);
...
int Card_Root_Size = 64; // adjustable
int Attack_Times_Size = 1;
int Minion_Type_Size = 3;
int Ability_Size = 7;
int Damage_Ability_Size = 2;
int Special_Effects_Size = 64; // adjustable
int Single_Effect_Size = 32; // adjustable
int Alle_Cond_Size = 3;
int Targeted_Play_Eff_Size = Single_Effect_Size + 3;
int Other_Eff_Size = Single_Effect_Size + Alle_Cond_Size + 6;
int Effect_Seq_Size = 64; // adjustable
int Base_Targeted_Eff_Size = 48; // adjustable
int Base_Untargeted_Eff_Size = 64; // adjustable
int Cond_Size = 16; // adjustable
int Sub_Cond_Size = 16; // adjustable
int Char_Type_Cond_Size = 5;
int Card_Type_Cond_Size = 7;
int Card_Pos_Cond_Size = 3;
int Abil_Cond_Size = 8;
int Stat_Cond_Variant_Size = 3;
int Stat_Cond_Size = 5 + Stat_Cond_Variant_Size;
int Eff_Utility_Size = 24; // adjustable; part of the rules from BaseTargtedEff
int Destination_Size = 6 + Alle_Cond_Size;
int New_Card_Variant_Size = 6 + 1 + Card_Root_Size;

int Hidden_Total_Partic_Size = 32;
int Hidden_Win_Contrib_Size = 32;
int Transformer_Unit_Size = 64;
int Hidden_Transformer_Size = 128;
