...
Player::Player(queue<DeferredEvent*>& _event_queue) :
	...
	turn_num(0), max_mp(0), mp_loss(0), fatigue(0), field_size_adjust(0), hand_size_adjust(0), deck_size_adjust(0), event_queue(_event_queue), ai_level(0), input_func(&Player::TakeInputs), card_pool(), encoder(nullptr)
{
}

Player::Player(const string& _name, int _hp, const vector<Card*>& _deck, bool _is_guest, queue<DeferredEvent*>& _event_queue, CardEncoderNet* _encoder) :
	...
	turn_num(0), max_mp(0), mp_loss(0), fatigue(0), field_size_adjust(0), hand_size_adjust(0), deck_size_adjust(0), event_queue(_event_queue), ai_level(0), input_func(&Player::TakeInputs), card_pool(), encoder(_encoder)
{
...
Player::Player(const string & _name, int _hp, const vector<Card*>& _deck, bool _is_guest, queue<DeferredEvent*>& _event_queue, int _ai_level, CardEncoderNet* _encoder) : Player(_name, _hp, _deck, _is_guest, _event_queue, _encoder)
{
	ai_level = _ai_level;
...
Player::Player(const string & _name, int _hp, const vector<Card*>& _deck, bool _is_guest, queue<DeferredEvent*>& _event_queue, int _ai_level, const vector<Card*>& _card_pool, CardEncoderNet* _encoder) : Player(_name, _hp, _deck, _is_guest, _event_queue, _ai_level, _encoder)
{
...
Card* Player::GetRandomUnknownCard() const
{
	...
	if (card_pool_size > 0)
		...
		return GenerateSampledCard("Anonymous", encoder);
	else
...
Card* CreateCardFromLatentCode(const string& name, const torch::Tensor& code, CardEncoderNet* encoder)
{
	return CreateNamedCardFromLatentCode(name, encoder, code);
}

Card* GenerateSampledCard(const string& name, CardEncoderNet* encoder)
{
	torch::Tensor code;
	if (encoder) // it'll still work without this "if" check, but just in case, better to not consume random numbers when not needed
		code = torch::randn({ Card_Root_Size }, torch::kDouble);
	return CreateCardFromLatentCode(name, code, encoder);
}

vector<Card*> GenerateSampledCardSet(int n, CardEncoderNet* encoder)
{
	vector<Card*> cards(n);

	for (int i = 0; i < n; i++)
		cards[i] = GenerateSampledCard("Anonymous", encoder);

	return cards;
}

Card* CreateCardFromLatentCodeWithCost(const string& name, const torch::Tensor& code, int cost, CardEncoderNet* encoder)
{
	return CreateNamedCardFromLatentCodeWithCost(name, encoder, code, cost);
}

Card* GenerateSampledCardWithCost(const string& name, int cost, CardEncoderNet* encoder)
{
	torch::Tensor code;
	if (encoder) // it'll still work without this "if" check, but just in case, better to not consume random numbers when not needed
		code = torch::randn({ Card_Root_Size }, torch::kDouble);
	return CreateCardFromLatentCodeWithCost(name, code, cost, encoder);
}

Card* CreateMinionFromLatentCodeWithCost(const string& name, const torch::Tensor& code, int cost, CardEncoderNet* encoder)
{
	return CreateNamedMinionFromLatentCodeWithCost(name, encoder, code, cost);
}

Card* GenerateSampledMinionWithCost(const string& name, int cost, CardEncoderNet* encoder)
{
	torch::Tensor code;
	if (encoder) // it'll still work without this "if" check, but just in case, better to not consume random numbers when not needed
		code = torch::randn({ Card_Root_Size }, torch::kDouble);
	return CreateMinionFromLatentCodeWithCost(name, code, cost, encoder);
}
