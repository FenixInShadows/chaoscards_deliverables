...
struct ExtraCardGenConfig
{
	...
	ExtraCardGenConfig(const std::string& _name, NodeRep*& _rep, int _min_cost, int _max_cost, bool _forcing_minion = false);
	std::string name;
	NodeRep*& rep; // note: sometimes the rep is merely a placehold but even in that case it should still be the reference to a variable that lives until the use of this config is done
	int min_cost;
	int max_cost;
	bool forcing_minion;
};

ExtraCardGenConfig* MkExtraCardGenConfig(const std::string& name, NodeRep*& rep);
ExtraCardGenConfig* MkExtraCardGenConfigWithCost(const std::string& name, NodeRep*& rep, int cost, bool forcing_minion = false); 
